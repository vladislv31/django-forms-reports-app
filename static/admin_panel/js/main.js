...
function create_questionnaire() {
    ...
    // delete field button
    $(document).on('click', '.create-questionnaire__item-field__delete', function() {
        const th = $(this);
        const field = th.closest('.create-questionnaire__item-field');

        field.remove();
    });

    // delete item button
    $(document).on('click', '.create-questionnaire__item-delete', function() {
        const th = $(this);
        const item = th.closest('.create-questionnaire__item');

        item.remove();
    });

    // add field link
    $(document).on('click', '.create-questionnaire__item-add-field', function(e) {
        e.preventDefault();

        const th = $(this);
        const item = th.closest('.create-questionnaire__item');
        const fields = $('.create-questionnaire__item-fields', item);

        const item_id = item.attr('data-item-id');

        const last_field = $('.create-questionnaire__item-field', fields).last();
        let id = 1;

        if (last_field?.length) id = Number(last_field.attr('data-field-id')) + 1;

        fields.append(`
            <div class="create-questionnaire__item-field" data-field-id="${id}">
                <div class="create-questionnaire__item-field__number">
                    <input type="text" class="form-control" name="${item_id}_${id}_number" placeholder="Номер" required>
                </div>
                <div class="create-questionnaire__item-field__question">
                    <input type="text" class="form-control" name="${item_id}_${id}_question" placeholder="Вопрос" required>
                </div>
                <div class="create-questionnaire__item-field__delete">
    ...
    // add item link
    $(document).on('click', '.create-questionnaire__add-item', function(e) {
        e.preventDefault();
        const items = $('.create-questionnaire__items');

        const last_item = $('.create-questionnaire__item', items).last();
        let id = 1;
        ...
        items.append(`
            <div class="create-questionnaire__item block" data-item-id="${id}">
                <div class="create-questionnaire__item-delete">
                    <img src="/static/main/images/delete.svg">
                </div>
                ...
                <hr>
                <div class="create-questionnaire__item-fields">
                    <label class="form-label">Поля меры обеспечения безопасности значимого объекта</label>
                </div>
                <hr>
                <div class="create-questionnaire__item-add-field">
                    <a class="link-info" href="#">Добавить вопрос</a>
                </div>
            </div>`);
    });

    $('form#create-questionnaire-form').submit(function(e) {
        e.preventDefault();
